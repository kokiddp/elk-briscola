...
- ~~**Testcontainers Postgres.**~~ **Closed during Phase 5.1:** Docker became available in the dev environment, so the dev/test database was switched to Postgres. `PostgresContainerPool` boots one Postgres 17 container per `dotnet test` run and hands each `BriscolaApiFactory` its own database via `CREATE DATABASE`. The Phase 3 SQLite repo tests under `Persistence/` stay on SQLite so both providers remain exercised. Surfaced one real bug en route: `LobbyService.CreateAsync` was writing `string.Empty` into the `StateSnapshotJson` column, which Postgres `jsonb` rejects with `22P02`; fixed by writing `"{}"` for Open games (the value is overwritten the moment the game transitions to Running). SQLite-only testing had hidden this.
...
- ~~**API auto-docs (REST + SignalR).**~~ **Closed in `feat/api-autodocs` (post-5.7).** Swashbuckle was already wired but the experience was bare; this pass: enabled XML doc generation on `Briscola.Api`; added action-level `<summary>` / `<remarks>` / `<response>` comments to every controller; registered the JWT bearer security scheme so both UIs get an "Authorize" button; added [Scalar](https://github.com/scalar/scalar) as a second UI at `/scalar` (the modern one, with built-in code samples in curl / JS / Python / C# / Go / …); hand-authored `backend/src/Briscola.Api/docs/asyncapi.json` covering both hubs and served it at `/docs/asyncapi.json` plus an HTML viewer at `/docs/asyncapi` (the viewer pulls `@asyncapi/react-component` from a CDN — degrades gracefully offline). All routes are dev-only; production stays surface-area-minimal. Smoke tests (`AutoDocsTests.cs`) assert dev=200 / prod=404 for every doc route. Test-factory got an `Environment` knob so individual tests can boot the host in `Development` without affecting the rest of the suite.
